...
import com.github.lkapitman.core.api.download.data.Data;
import com.github.lkapitman.core.api.download.data.DataFile;
import com.github.lkapitman.core.api.download.hash.HashUtil;
...
public class Updater {
    ...
    public void start() {
        ...
        for (int i = 0; i < length; i++){
            ...
            if (!downloadManager.getFile().exists()) {
                ...
                Core.getLogger().INFO_LOG("Folder " + downloadManager.getFile().getName() + " doesn't exist, mkdirs folder.");
            } else {
                Core.getLogger().INFO_LOG("Folder " + downloadManager.getFile().getName() + " exist, start check url.");
            }
    ...
    private void jsonDeserialize(String str, DownloadJob job, File dir) {
        ...
        for (DataFile file : data.getFiles()) {
            ...
            try {
                if(!HashUtil.getMD5(client_file).equals(file.getHash())) {
                    Core.getLogger().INFO_LOG("File " + client_file.getName() + " has a bad hash '" + HashUtil.getMD5(client_file) + "' hash on server => " + file.getHash());
                    job.addDownloadable(client_file, file.getHash(), file.getUrl());
                    this.ignoredFiles.add(client_file.getAbsolutePath());
                }else{
                    this.ignoredFiles.add(client_file.getAbsolutePath());

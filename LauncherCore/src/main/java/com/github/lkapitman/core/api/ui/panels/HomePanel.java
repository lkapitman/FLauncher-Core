...
import com.github.lkapitman.core.api.download.tasks.DownloadListener;

import com.github.lkapitman.core.api.elements.AProgressBar;
...
public class HomePanel extends Panel implements DownloadListener {
    ...
    private void addTopPanel(GridPane pane) {
        ...
        resultButton.setOnMouseClicked(e-> {
            try {
                String cmd = s.replaceAll("C:/Users/Lion/AppData/Roaming/.minecraft", this.panelManager.getCore().getFileManager().getGameFolder().getAbsolutePath());
                cmd = s.replaceAll("Aziat", this.panelManager.getCore().getFileManager().getGameFolder().getAbsolutePath());
                Runtime.getRuntime().exec(cmd);
            } catch (IOException ex) {
                ex.printStackTrace();
            }
            this.panelManager.getCore().launchGame();
        });
    ...
    public void onDownloadJobFinished(DownloadJob job) {
        Core.getLogger().INFO_LOG(job.getName() + " finished!");
    ...
    public void onDownloadJobStarted(DownloadJob job) {
        Core.getLogger().INFO_LOG("'" + job.getName() + "' started to download!");

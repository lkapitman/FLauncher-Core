...
import com.github.lkapitman.downloader.download.data.Data;
import com.github.lkapitman.downloader.download.data.DataFile;
    ...
    public void start() {
        ...
        for (int i = 0; i < length; i++){
            ...
            if (!downloadManager.getFile().exists()) {
                ...
                System.out.println("Folder " + downloadManager.getFile().getName() + " doesn't exist, mkdirs folder.");
            } else {
                System.out.println("Folder " + downloadManager.getFile().getName() + " exist, start check url.");
            }
        ...
        for (DataFile file : data.getFiles()) {
            ...
            try {
                if(!HashUtil.getMD5(client_file).equals(file.getHash())) {
                    System.out.println("File " + client_file.getName() + " has a bad hash '" + HashUtil.getMD5(client_file) + "' hash on server => " + file.getHash());
                    job.addDownloadable(client_file, file.getHash(), file.getUrl());
                    this.ignoredFiles.add(client_file.getAbsolutePath());
                }else{
                    this.ignoredFiles.add(client_file.getAbsolutePath());
